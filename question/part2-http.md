...
> 문진수
>
> Port는 동일 호스트 내에서 프로세스를 식별하는 용도로 사용되는 정수값입니다 .0~1023은 Well-Known Port로 이미 할당되어 있습니다. (HTTP(80), HTTPS(443), SSH(22) 등등..)
>
> 하나의 프로세스는 다수의 포트를 사용할 수 있지만 포트 하나에 여러 프로세스를 연결할 수는 없습니다.
...
> 문진수
>
> 소켓은 네트워크 상에서 두 프로세스 간 통신의 종착점을 의미합니다. Linux 운영체제에서는 소켓도 하나의 파일로 취급되며 파일 디스크립터를 통해 접근됩니다. 프로세스가 열 수 있는 파일의 개수는 시스템 설정에 따라 제한되며 소켓도 파일의 일종이기 때문에 이러한 제약이 동일하게 적용됩니다.
>
> TCP 소켓은 (프로토콜, src IP, src Port, dest IP, dest Port)로 유일하게 식별됩니다.
...
> 문진수
>
> 소켓은 네트워크 상에서 두 프로세스 간 통신의 종착점을 의미하고 Linux 운영체제에서는 파일입니다. 프로토콜, 출발지, 목적지 IP, Port로 구성됩니다. 반면 Port는 동일 호스트에서 프로세스와 연결된 일종의 data path입니다. port Number를 통해서 호스트에서 동작하고 있는 특정 프로세스를 식별할 수 있습니다.
...
> 문진수
>
> HTTP 프로토콜은 웹에서 데이터를 주고 받는 클라이언트-서버 모델의 프로토콜입니다. 요청-응답 모델을 기반으로 하고 요청은 요청 메소드, 프로토콜 버전, URL과 데이터의 메타데이터를 나타내는 헤더, 그리고 실제 데이터가 담기는 Body로 구성됩니다.
...
> 문진수
>
> HTTP 1.0에서는 content Type이 추가되면서 HTML이 아닌 다양한 데이터를 전송할 수 있게 됐습니다. 그리고 상태값이 추가됨에 따라 클라이언트에서 이를 확인하고 유연하게 처리할 수 있게 됐습니다. 1.1에서는 지속 연결을 지원함에 따라 하나의 TCP 연결을 재사용하여 여러번 데이터를 전송할 수 있게 됐습니다. 이에 따라 매번 3-way-handshake, 4-way-handshake를 하던 오버헤드가 줄어들었습니다. 또한 파이프라이닝 기능이 추가되어 응답을 받지 않고도 요청을 보낼 수 있게 됐습니다. 하지만 HTTP 1.1에서는 Req-Res쌍을 식별할 방법이 없어 애플리케이션 계층에서 순차적으로 처리해야 하기 때문에 HoL Blocking 문제가 발생합니다. HTTP 2.0에서는 이러한 문제를 해결하고자 Stream 방식을 도입하여 애플리케이션 계층에서의 HoL Blocking 문제를 해결했습니다. 하지만 여전히 TCP를 사용하기 때문에 전송 계층에서의 HoL Blocking 문제는 해결할 수 없습니다. 그리고 텍스트 방식이 아닌 binary frame으로 인코딩되면서 헤더도 압축할 수 있게 됐습니다. 그리고 Header와 Body를 기존에서는 \n, \r로 구분했는데 이제는 별도의 Layer로 구분해서 전송함으로써 데이터 파싱 속도가 늘어나고 오류 발생 가능성이 줄었습니다.
...
> 문진수
>
> QUIC은 HTTP 3.0에서 사용하는 전송 계층 프로토콜로 UDP를 기반으로 동작합니다. 기존 HTTP 2.0은 TCP를 사용하기 때문에 전송 계층에서 HoL Blocking 문제가 발생했지만 QUIC은 UDP 위에서 동작하며 자체적인 멀티플렉싱 기능을 제공해 이러한 문제를 해결합니다. 또한 QUIC은 Stream 단위로 데이터를 전송하므로 하나의 스트림에서 문제가 발생해도 다른 스트림에 영향을 주지 않습니다. 그리고 QUIC은 애플리케이션 계층에서 혼잡 제어, 흐름 제어, 신뢰성 보장 등의 기능을 직접 구현합니다.
...
> HoL Blocking이란 앞선 요청이나 데이터가 지연되면서 뒤따르는 요청이나 데이터의 처리까지 함께 지연되는 현상을 말합니다. HTTP 1.1에서는 파이프라이닝을 도입했지만 요청과 응답을 1:1로 정확히 매핑할 방법이 없어 응답은 요청 순서대로 처리될 수밖에 없었습니다. 이로 인해 하나의 요청이 지연되면 뒤의 요청들도 함께 지연되어 HoL Blocking 문제가 발생했습니다. HTTP 2.0에서는 멀티플렉싱을 지원하는stream개념을 도입해 응용 계층에서의 HoL Blocking 문제는 해결했습니다. 하지만 전송 계층에서 TCP를 사용하기에 앞선 세그먼트가 도착하지 않으면 뒤의 요청이 처리되지 못하고 여전히 HoL Blocking 문제가 발생합니다.
...
> 문진수
>
> 200번대는 성공 응답으로 별도의 응답이 없는 경우인 204 NO CONTENT, 자원이 생성됨을 나타내는 201 CREATED 등이 있습니다.
>
> 300번대는 Redirect을 의미하고 요청을 완료하려면 추가적인 작업이 필요함을 의미합니다. Redirect URL이 응답에 포함되고 해당 URL로 다시 요청을 하여 처리하게 됩니다.
>
> 400번대는 클라이언트 오류를 말하며 잘못된 메시지를 보내거나 존재하지 않는 URL 등의 요청이 있다.
>
> 500번대는 서버 오류를 의미한다.
